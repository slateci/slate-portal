...
def check_user_exists():
    ...
    response = requests.get(slate_api_endpoint + "/v1alpha3/find_user", params=query)
    return response
...
def list_cluster_whitelist(cluster_name):
    ...
    response = requests.get(
        slate_api_endpoint + "/v1alpha3/clusters/" + cluster_name + "/allowed_groups",
        params=query,
    )
...
def list_user_groups(session):
    ...
    app.logger.debug(
        "Querying to get user groups with user id: {}".format(slate_user_id)
    )
...
def cluster_exists(cluster_name):
    ...
    app.logger.debug(
        "Checking if cluster {} exists in current clusters...".format(cluster_name)
    )
    cluster_names = []
    ...
    if cluster_name not in cluster_names:
        app.logger.debug(
            "Returning False because did not find {} in {}".format(
