...
def dashboard():
    ...
    if request.method == "GET":
        ...
        return render_template(
            "dashboard.html", clusters=clusters, news=news, clusters_list=clusters_list
        )
...
def view_admin():
    if request.method == "GET":
        ...
        users = requests.get(slate_api_endpoint + "/v1alpha3/users", params=query)
...
def cli_access():
    if request.method == "GET":
        ...
        try:
            ...
            query = {"token": slate_api_token, "globus_id": globus_id}

            r = requests.get(slate_api_endpoint + "/v1alpha3/find_user", params=query)
            user_info = r.json()
...
def mailgun(group_name, user_name, user_email):
    ...
    r = requests.post(
        "https://api.mailgun.net/v3/slateci.io/messages",
        ...
        data={
            ...
            "text": "Hello! User, {}, is CC'd on this email {}, and has requested to join your group, {} on SLATE. You may choose to add them from your groups detailed page through the SLATE portal.".format(
...
def secret_select_group():
    if request.method == "GET":
        ...
        s = requests.get(
            slate_api_endpoint + "/v1alpha3/users/" + slate_user_id + "/groups",
            params=query,
        )
...
def view_group(name):
    if request.method == "GET":
        return render_template(
            "groups_profile_overview.html", name=name, minislate_user=minislate_user
        )
        ...
        r = requests.delete(
            slate_api_endpoint + "/v1alpha3/clusters/" + cluster_name, params=query
        )
...
def group_admin_clusters_ajax(group_name):
    administering_clusters, accessible_clusters_diff = group_admin_clusters_request(
        group_name
    )
...
def group_admin_clusters_request(group_name):
    ...
    administering_clusters = requests.get(
        slate_api_endpoint + "/v1alpha3/groups/" + group_name + "/clusters",
        params=query,
    )
    administering_clusters = administering_clusters.json()["items"]
    administering_clusters_names = [
        administering_cluster["metadata"]["name"]
        for administering_cluster in administering_clusters
    ]
    ...
    accessible_clusters_names = [
        accessible_cluster["metadata"]["name"]
        for accessible_cluster in accessible_clusters
    ]
    accessible_clusters_diff = list(
        set(accessible_clusters_names) - set(administering_clusters_names)
    )
...
def view_group_members(name):
    ...
    if request.method == "GET":
        ...
        non_members = [
            user["metadata"]
            for user in users
            if user["metadata"]["id"] not in group_member_ids
        ]
...
def view_group_add_members(name):
    ...
    if request.method == "GET":
        ...
        return render_template(
            "groups_profile_add_members.html",
            users=users,
            name=name,
            group_info=group_info,
            non_members=non_members,
...
def view_group_volumes(name):
    if request.method == "GET":
        ...
        query = {"token": access_token}
        r = requests.delete(
            slate_api_endpoint + "/v1alpha3/volumes/" + volume_id, params=query
        )
        if r.status_code == requests.codes.ok:
...
def view_group_secrets(name):
    if request.method == "GET":
        return render_template(
            "groups_profile_secrets.html", name=name, minislate_user=minislate_user
        )
    elif request.method == "POST":
        ...
        r = requests.delete(
            slate_api_endpoint + "/v1alpha3/secrets/" + secret_id, params=secrets_query
        )
...
def create_secret(name):
    ...
    if request.method == "GET":
        ...
        for key_name, key_contents in zip(
            request.form.getlist("key_name"), request.form.getlist("key_contents")
        ):
...
def create_group_volume(name):
    ...
    if request.method == "GET":
        return render_template(
            "group_volumes_create.html", name=name, minislate_user=minislate_user
        )
    elif request.method == "POST":
        # Initialize empty contents dict
        contents = {}
...
@app.route(
    "/groups/<project_name>/clusters/<name>/<group_name>", methods=["GET", "POST"]
)
@authenticated
def group_cluster_apps(project_name, name, group_name):
    ...
    if request.method == "GET":
        ...
        multiplex_JSON = {
            group_apps_query: {"method": "GET"},
            applications_query: {"method": "GET"},
        }
        # POST request for multiplex return
        multiplex = requests.post(
            slate_api_endpoint + "/v1alpha3/multiplex",
            params=query,
            json=multiplex_JSON,
        )
        ...
        if r.status_code == requests.codes.ok:
            ...
            err_message = r.json()["message"]
            flash(
                "Failed to add applications to group: {}".format(err_message), "warning"
            )
        return redirect(url_for("view_cluster", project_name=project_name, name=name))


@app.route(
    "/groups/<project_name>/clusters/<name>/remove_group_from_cluster", methods=["POST"]
)
@authenticated
...
def edit_cluster(project_name, name):
    ...
    cluster_id = name
    cluster = requests.get(
        slate_api_endpoint + "/v1alpha3/clusters/" + cluster_id, params=query
    )
    cluster = cluster.json()
    ...
    if request.method == "GET":
        ...
        for latitude, longitude in zip(
            request.form.getlist("latitude"), request.form.getlist("longitude")
        ):
            try:
                address = coordsConversion(latitude, longitude)
                locations.append(
                    {"lat": float(latitude), "lon": float(longitude), "desc": address}
                )
            except:
        ...
        r = requests.put(
            slate_api_endpoint + "/v1alpha3/clusters/" + cluster_id,
            params=query,
            json=add_cluster,
        )
...
def delete_group(name):
    if request.method == "POST":
        ...
        try:
            ...
            app.logger.error(
                "Timed out deleting group {}, but force to next page".format(name)
            )
...
def edit_group(name):
    ...
    if request.method == "GET":
        ...
        r = requests.put(
            slate_api_endpoint + "/v1alpha3/groups/" + name,
            params=query,
            json=add_group,
        )
        ...
        if r.status_code == requests.codes.ok:
            ...
            err_message = r.json()["message"]
            flash(
                "Failed to update group information: {}".format(err_message), "warning"
            )
            return redirect(url_for("list_groups"))
...
def create_profile():
    ...
    if request.method == "GET":
        ...
        if aup_check:
            ...
            r = requests.post(
                slate_api_endpoint + "/v1alpha3/users", params=query, json=post_user
            )
            if r.status_code == requests.codes.ok:
                user = r.json()
...
def profile():
    ...
    if request.method == "GET":
        ...
        user = requests.get(slate_api_endpoint + "/v1alpha3/find_user", params=query)

        if user:
            slate_id = user.json()["metadata"]["id"]
            profile = requests.get(
                slate_api_endpoint + "/v1alpha3/users/" + slate_id, params=query
            )
...
def edit_profile():
    if request.method == "GET":
        identity_id = session.get("primary_identity")
        access_token = get_user_access_token(session)
        query = {"token": slate_api_token, "globus_id": identity_id}
        ...
        if profile:
            ...
            app.logger.debug(
                "Response from querying profile details: {}".format(profile)
            )
            profile = profile.json()["metadata"]
        else:
            flash("Please complete any missing profile fields and press Save.")
        ...
        requests.put(
            slate_api_endpoint + "/v1alpha3/users/" + slate_user_id,
            params=query,
            json=put_user,
        )
...
def authcallback():
    ...
    if slate_portal_user:
        ...
        app.logger.debug(
            "Redirecting to dashboard in single user mode with the following session {}".format(
                session
            )
        )
        return redirect(url_for("dashboard"))
    ...
    redirect_uri = url_for(
        "authcallback", _external=True, _scheme=app.config["GLOBUS_REDIRECT_URI_SCHEME"]
    )
    ...
    if "code" not in request.args:
        ...
        for identity in identity_set:
            query = {"token": slate_api_token, "globus_id": identity}
            try:
                ...
                if r.status_code == requests.codes.ok:
                    ...
                    app.logger.debug(
                        "Found profile with globus_id: {}".format(identity)
                    )
                    profile = r.json()
...
def create_cluster():
    ...
    if request.method == "GET":
        ...
        return render_template(
            "clusters_create.html", group_list=group_list, minislate_user=minislate_user
        )
...
def list_secrets():
    ...
    if request.method == "GET":
        ...
        for group_name in user_groups:
            secrets_query = (
                "/v1alpha3/secrets?token=" + query["token"] + "&group=" + group_name
            )
...
def list_volumes():
    ...
    if request.method == "GET":
        access_token, slate_user_id = get_user_info(session)
        query = {"token": access_token}
        volumes = requests.get(slate_api_endpoint + "/v1alpha3/volumes", params=query)
...
def volume_info(volume_id):
    ...
    if request.method == "GET":
        ...
        if response.status_code == 504:
            ...
            return render_template(
                "volume_profile.html", name=volume_id, volume_details=volume_details
            )
...
def create_volume():
    ...
    if request.method == "GET":
        groups = list_user_groups(session)
        return render_template(
            "volume_create.html", groups=groups, minislate_user=minislate_user
        )
    elif request.method == "POST":
        # Initialize empty contents dict
        contents = {}
